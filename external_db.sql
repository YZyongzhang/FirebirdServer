...
-- 1. 为users表添加余额字段（充值功能）
ALTER TABLE users ADD COLUMN balance DECIMAL(12,2) DEFAULT 0;

-- 2. 为users表添加审核状态字段（开店审核功能）
ALTER TABLE users ADD COLUMN status VARCHAR(50) DEFAULT 'approved';
